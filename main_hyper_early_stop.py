...
class HyperOptManager:
    ...
    def __init__(self, year: int, val_ont_hops: Optional[int], eval_patience: int = 30):
        self.year = year
        self.n_epochs = 20
        self.val_ont_hops = val_ont_hops
        self.eval_patience = eval_patience  # Number of evaluations to wait for an improvement

        self.eval_num = 0
        self.no_improvement_eval_count = 0  # Counter for evaluation stopping
        self.best_loss = None
    ...
    def run(self):
        ...
        best = fmin(self.objective, space=space, algo=tpe.suggest, max_evals=100, trials=self.trials, show_progressbar=False)
    ...
    def objective(self, hyperparams):
        ...
        for epoch in epochs_progress:
            ...
            if best_accuracy is None or validation_accuracy > best_accuracy:
                ...
                self.no_improvement_eval_count = 0  # Reset the counter when improvement is observed
            else:
                self.no_improvement_eval_count += 1  # Increment the counter when no improvement

            # Print the count of evaluations with no improvement
            print(f"No improvement evaluations count: {self.no_improvement_eval_count}")

            # Check for evaluation stopping
            if self.no_improvement_eval_count >= self.eval_patience:
                print(f"Stopping hyperparameter optimization due to no improvement in {self.eval_patience} consecutive evaluations")
                return {
